...
for fq1 in $fastqDir/*_1.fastq.gz; do
	base=`basename $fq1`
	fq2=${fq1/_1.fastq.gz/_2.fastq.gz}; # the R1 pattern might be different!
	out=${base/_1.fastq.gz/};
	echo $out;
	bwa mem -t $JOBS ${genomeDir}/${genomeRef}.fa $fq1 $fq2 | samtools view -bS - &> ${bamDir}/${out}.${genomeRef}.bam;
	cd $bamDir
